...
#################################################
#   Global variables
InputFilePath = 'problem_018.txt'

#################################################
#   Functions
def getAnswer( filePath ):
	""" Get answer of given file
	"""
	...
	with open( filePath, 'rb' ) as inputFile:
		...
		for line in inputFile.readlines():
			...
			listData = list()
			for data in strippedLine.split():
		...
		for i in range(0, len(dataList) - 1):
			j = 0
			for k in dataList[i+1]:
				dataList[i+1][j] = max( [int( k ) + int( dataList[i][j] ), 
										int( k ) + int( dataList[i][j+1] ) ] )
				# print k, dataList[i+1][j], int(dataList[i+1][j])-int(k) 
				j += 1
		
		return dataList[len(dataList)-1][0]
